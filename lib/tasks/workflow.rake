namespace :workflow do
  ...
  task :diagrams, [:class, :path] => :environment do |t, args|
...
def create_workflow_diagram(workflow_name, steps, target_dir='.', graph_options='rankdir="TD", size="7,11.6", ratio="fill"')
...
digraph #{workflow_name} {
  ...
  edge [len=1];
    ...
    steps.each_pair do |step, outcomes|
      ...
      file.puts %Q{  "#{normalized_step}" [label="#{normalized_step}"];}

      outcomes.step_map.each_pair do |response, next_step|
        file.puts %Q{  "#{normalized_step}" -> "#{normalize_step(next_step)}" [label="#{normalize_step(response)}"];}
      end
    end
    file.puts "}"
    file.puts
